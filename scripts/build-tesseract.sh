...
if [ "$(uname -s)" != "Darwin" ] && command -v patchelf >/dev/null 2>&1; then
    for so in "$PREFIX/lib/libtesseract.so."*; do
        if [ -f "$so" ] && [ ! -L "$so" ]; then
            ...
            # to liblept.so.5 to match the SONAME we now force on the
            # bundled Leptonica (see build-leptonica.sh — patchelf --set-soname
            # liblept.so.5). Both sides of the pair must speak the same
            # SONAME or the dynamic linker won't consider our lib when
            # resolving our tesseract's NEEDED.
            patchelf --replace-needed libleptonica.so.6 liblept.so.5 "$so"
            echo "==> patchelf --replace-needed libleptonica.so.6 liblept.so.5 $so"
        fi
    done
fi
